...
| Per-user network helper and background jobs (v1.2) | Native TUN/UDP helper round trip and cleanup, plus current-user and administrator service lifecycle tests passed on Linux amd64/arm64, macOS arm64 and Windows amd64 in [CI 34772505292](https://github.com/k0ngk0ng/wire-connect/actions/runs/34772505292). Windows uses a real non-elevated user token; first-time adapter setup waits for usable IPv4 addressing |
| Windows self-update handoff and interrupted replacement | Native Windows tests exercise replacement of the running executable, runtime sidecars, refresh and staging cleanup. Interrupted replacement is resumed through its explicit transaction entry point; there is no boot-time transaction scanner |
