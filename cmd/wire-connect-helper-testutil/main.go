...
// Unix network helper.  It is built only for the opt-in integration test; it
// deliberately refuses to run as root so that the test cannot accidentally
// exercise the native TUN path in the child process.
package main
...
func run(args []string) error {
	if os.Geteuid() == 0 {
		return errors.New("network helper integration child must run without root privileges")
	}
